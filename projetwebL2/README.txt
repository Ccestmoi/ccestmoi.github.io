## Attributs et Description des Questions

Chaque question est définie par les balises suivantes :
...
### Description
La fonction `loadXMLDoc1` est utilisée pour charger un document XML contenant les questions de la base de données. Elle envoie une requête HTTP pour récupérer le fichier XML et appelle des fonctions spécifiques en fonction de la page actuelle.

### Fonctionnement
1. Crée une nouvelle instance de `XMLHttpRequest`.
2. Définit une fonction de rappel pour gérer l'état de la requête.
3. Si la requête est terminée avec succès (`readyState == 4` et `status == 200`), elle vérifie la page actuelle :
   - Si la page est `detail.html`, elle appelle la fonction `displayQuestionById`.
   - Sinon, elle appelle la fonction `fetchData`.
4. Envoie la requête HTTP pour récupérer le fichier XML à partir de l'URL spécifiée.


## Fonction `fetchData`

### Description
La fonction `fetchData` est utilisée pour extraire les données des questions du document XML chargé et les stocker dans un tableau. Elle affiche également les questions si la page actuelle est `apprendre.html`.

### Fonctionnement
1. Initialise un tableau vide `data`.
2. Récupère le document XML de la réponse de la requête HTTP.
3. Parcourt tous les éléments `<question>` du document XML et les ajoute au tableau `data`.
4. Si la page actuelle est `apprendre.html`, elle appelle les fonctions `showPageLinks` et `displayData` pour afficher les questions.


## Fonction `displayData`

### Description
La fonction `displayData` est utilisée pour afficher les questions de la base de données XML dans un tableau HTML sur la page `apprendre.html`. Elle gère également la pagination des questions.
...
### Description
La fonction `filtrage` est utilisée pour filtrer les questions affichées sur la page `apprendre.html` en fonction des critères sélectionnés par l'utilisateur, tels que le thème et le niveau de difficulté.

### Fonctionnement
1. Récupère les valeurs des filtres sélectionnés par l'utilisateur (thème et niveau).
2. Initialise une variable `filteredData` pour stocker les questions filtrées.
3. Parcourt toutes les questions dans le tableau `data` et vérifie si elles correspondent aux critères de filtrage.
4. Si une question correspond aux critères, elle est ajoutée à `filteredData`.
5. Met à jour le tableau `data` avec les questions filtrées.
6. Appelle la fonction `displayData` pour afficher les questions filtrées.
...
### Fonctionnement
1. Réinitialise les valeurs des filtres de thème et de niveau à "all".
...
### Description
La fonction `triPoints` est utilisée pour trier les questions en fonction du nombre de points attribués à chaque question. Elle est utilisée pour organiser les questions par ordre croissant ou décroissant de points.
...
### Description
La fonction `sortData` est utilisée pour trier les questions affichées sur la page `apprendre.html` en fonction du critère de tri sélectionné par l'utilisateur, tel que le nombre de points ou le niveau de difficulté.
...
### Description
La fonction `loadPage` est utilisée pour changer la page des questions affichées sur la page `apprendre.html`. Elle permet de naviguer entre les différentes pages de questions en fonction du numéro de page sélectionné.
...
### Description
La fonction `showPageLinks` est utilisée pour afficher les liens de pagination sur la page `apprendre.html`. Elle permet de naviguer entre les différentes pages de questions.

### Fonctionnement
1. Récupère l'élément HTML avec l'ID `pageLinks` et le configure pour être affiché en mode flex.
...
### Description
La fonction `displayQuestionById` est utilisée pour afficher les détails d'une question spécifique sur la page `detail.html`. Elle récupère l'identifiant de la question à partir de l'URL et affiche le contenu de la question, les réponses possibles et l'explication.
...
### Description
La fonction `Niveau` est utilisée pour définir le niveau de difficulté du jeu et initialiser les paramètres en conséquence. Elle affiche également les éléments de l'interface utilisateur nécessaires pour commencer le jeu.

### Fonctionnement
1. Définit le niveau de difficulté (`niveau`) et le temps alloué (`time`) en fonction du paramètre `n` passé à la fonction.
2. Si le niveau est "1", affiche l'élément HTML avec l'ID `validation`.
3. Cache l'élément HTML avec l'ID `niveau`.
4. Affiche les éléments HTML avec les IDs `timer` et `question`.
...
### Description
La fonction `startjeu` est utilisée pour démarrer le jeu en chargeant les questions en fonction des paramètres de l'URL. Elle initialise également les variables nécessaires pour le jeu.

### Fonctionnement
1. Récupère les paramètres `theme` et `lot` de l'URL.
2. Si un thème est spécifié, charge les questions correspondant à ce thème et met à jour le score maximum (`maxscore`).
3. Si un lot est spécifié, charge les questions correspondant à ce lot et met à jour le score maximum (`maxscore`).
4. Appelle la fonction `displayQuestion` pour afficher la première question.
...
### Description
La fonction `timer` est utilisée pour gérer le compte à rebours du jeu. Elle décrémente le temps restant à intervalles réguliers et met à jour l'affichage du timer. Lorsque le temps est écoulé, elle arrête le timer et passe à la question suivante.
...
### Description
La fonction `displayQuestion` est utilisée pour afficher la question actuelle et ses réponses possibles dans le jeu. Elle gère également le timer pour chaque question et vérifie si le jeu est terminé.
...
## Fonction `selectionReponse`

### Description
La fonction `selectionReponse` est utilisée pour gérer la sélection d'une réponse par l'utilisateur dans le jeu. Elle met à jour l'interface utilisateur pour indiquer la réponse sélectionnée.
...
### Description
La fonction `validateAnswer` est utilisée pour valider la réponse sélectionnée par l'utilisateur. Elle appelle la fonction `checkAnswer` avec la réponse sélectionnée pour vérifier si elle est correcte.
...
### Description
La fonction `checkAnswer` est utilisée pour vérifier si la réponse sélectionnée par l'utilisateur est correcte. Elle met à jour le score, affiche un indicateur visuel de la réponse correcte ou incorrecte, et passe à la question suivante après un court délai.
...
### Description
La fonction `correction` est utilisée pour créer un tableau de correction affichant les questions, les réponses correctes, les réponses données par l'utilisateur et les explications pour chaque question du quiz.
...
5. Affiche le tableau de correction dans l'élément HTML avec l'ID `table-correction`.


## Fonction `endGame`

### Description
La fonction `endGame` est utilisée pour gérer la fin d'une partie de quiz. Elle enregistre le score et la date de la partie dans l'historique, affiche le tableau de correction et met à jour l'interface utilisateur pour montrer l'écran de fin de jeu.
...
### Description
La fonction `loadStats` est utilisée pour charger et afficher les statistiques des parties jouées. Elle calcule et affiche le nombre total de parties jouées ainsi que le score moyen des joueurs.
...
### Description
La fonction `updateHistogram` est utilisée pour mettre à jour l'affichage de l'histogramme des statistiques. Elle gère l'affichage du mois et de l'année en cours ainsi que les boutons de navigation temporelle.
...
### Description
La fonction `prevMonth` est utilisée pour naviguer vers le mois précédent dans l'affichage de l'histogramme des statistiques. Elle gère également le changement d'année lorsque nécessaire.
...
### Description
La fonction `nextMonth` est utilisée pour naviguer vers le mois suivant dans l'affichage de l'histogramme des statistiques. Elle gère également le changement d'année lorsque nécessaire.
...
### Description
La fonction `drawHistogram` est utilisée pour dessiner l'histogramme des scores dans un canvas HTML. Elle crée une représentation graphique des scores moyens par jour pour le mois sélectionné.
